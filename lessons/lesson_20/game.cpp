...
game::game()
{
  ...
  m_ball.add_horizontal_block(m_block_a);
  m_ball.add_horizontal_block(m_block_b);
  ...
  m_ball.add_vertical_block(m_block_c);
  m_ball.add_vertical_block(m_block_d);

  x = ( SDLEngine::instance().windowWidth() / 2 ) - m_ball.width() / 2; 
  y = ( SDLEngine::instance().windowHeight() / 2 ) - m_ball.height() / 2;
  m_ball.set_position(x, y);
}
//------------------------------------------------------------------------------
game::~game()
{
